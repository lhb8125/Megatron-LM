...
def initialize_gpt_model(
    ...
    **config_kwargs
    ...
    for i in range(virtual_pipeline_model_parallel_size or 1):
        ...
        if is_moe:
            layer_spec = layer_spec_fn(transformer_config, use_transformer_engine=True)
        else:
...
@pytest.mark.parametrize(
    ('src_tp_pp_vp', 'dst_tp_pp_vp', 'src_pp_layout', 'dst_pp_layout', 'is_moe'),
    [
        (
            (1, 4, 2),
            (1, 2, 1),
...
def test_save_and_load_checkpoint_vp(
    create_args,
    tmp_path_dist_ckpt,
    src_tp_pp_vp,
    src_pp_layout,
    ...
    def set_tp_pp_vp(tp, pp, vp=None, pp_layout=None, destroy_first=True):
        if destroy_first:
            Utils.destroy_model_parallel()
        Utils.initialize_model_parallel(tp, pp, vp)
    ...
    set_tp_pp_vp(*src_tp_pp_vp, pp_layout=src_pp_layout, destroy_first=False)
    ...
    with TempNamedDir(
        ...
        set_tp_pp_vp(*dst_tp_pp_vp, pp_layout=dst_pp_layout)
