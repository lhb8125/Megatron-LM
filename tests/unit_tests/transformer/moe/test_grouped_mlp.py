...
def test_expert_fc1_and_moe_act_recompute_use_single_checkpoint(monkeypatch):
    class FakeCheckpoint:
        ...
        def __init__(self, fp8=False):
            ...
            self.checkpoint_arg_count = None
            self.hook_tensor = None
            FakeCheckpoint.instances.append(self)

        def checkpoint(self, run_function, *args):
            self.checkpoint_name = run_function.__name__
            self.checkpoint_arg_count = len(args)
            return run_function(*args)

        def discard_output_and_register_recompute(self, hook_tensor):
            self.hook_tensor = hook_tensor
    ...
    class FakeLinear(torch.nn.Module):
        ...
        def forward(self, hidden_states, tokens_per_expert):
            ...
            if self.name == "fc1":
                self.last_output = hidden_states + 1.0
                return self.last_output, None
            self.last_output = hidden_states * 2.0
            return self.last_output, None

    monkeypatch.setattr(experts_module.tensor_parallel, "CheckpointWithoutOutput", FakeCheckpoint)
    ...
    module.expert_fc1_recompute = True
    module.expert_fc1_checkpoint = None
    module.activation_func = F.gelu
    module.linear_fc1 = FakeLinear("fc1")
    module.linear_fc2 = FakeLinear("fc2")

    hidden_states = torch.ones(2, 4, requires_grad=True)
    tokens_per_expert = torch.tensor([1, 1])
    permuted_probs = torch.ones(2, requires_grad=True)

    output, output_bias = module.forward(hidden_states, tokens_per_expert, permuted_probs)

    assert output_bias is None
    assert len(FakeCheckpoint.instances) == 1
    checkpoint = FakeCheckpoint.instances[0]
    assert checkpoint.checkpoint_name == "expert_fc1_and_moe_act_forward"
    assert checkpoint.checkpoint_arg_count == 2
    assert checkpoint.hook_tensor is module.linear_fc2.last_output
    torch.testing.assert_close(output, F.gelu(hidden_states + 1.0) * 2.0)
