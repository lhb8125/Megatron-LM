...
def run_model_a2a_overlap_with_capture(model, input_tensors, microbatches):
    ...
    dispatch_output, detached_outputs = callables.dispatch.forward(
        comm_stream, events[0], permutated_local_input_tokens
    )
    ...
    mlp_output, detached_outputs = callables.mlp.forward(
        comp_stream, events[0], global_input_tokens, tokens_per_expert, pre_mlp_layernorm_output
    )
    ...
    combine_output, detached_outputs = callables.combine.forward(
        comm_stream, events[0], expert_output
    )
    ...
    post_combine_output, detached_outputs = callables.post_combine.forward(
        comp_stream, events[0], expert_output, shared_expert_output, mlp_bias, hidden_states
    )
    ...
    for i in range(1, microbatches):
        ...
        callables.post_combine.backward(
            comp_stream,
            events[prev_idx],
            post_combine_outputs[prev_idx],
            torch.ones_like(post_combine_outputs[prev_idx]),
        )
        ...
        callables.combine.backward(
            ...
            *combine_outputs[prev_idx],
            combine_detached_outputs[prev_idx],
        )
        ...
        dispatch_output, detached_outputs = callables.dispatch.forward(
            comm_stream, events[i], permutated_local_input_tokens
        )
        global_input_tokens = detached_outputs[0]
        dispatch_outputs.append(dispatch_output)
        dispatch_detached_outputs.append(detached_outputs)
        ...
        callables.mlp.backward(
            comp_stream, events[prev_idx], *mlp_outputs[prev_idx], mlp_detached_outputs[prev_idx]
        )
        ...
        callables.dispatch.backward(
            ...
            *dispatch_outputs[prev_idx],
            dispatch_detached_outputs[prev_idx],
        )

        # f3. MLP forward for current microbatch
        mlp_output, detached_outputs = callables.mlp.forward(
            comp_stream, events[i], global_input_tokens, tokens_per_expert, pre_mlp_layernorm_output
        )
        expert_output, shared_expert_output, mlp_bias = detached_outputs
        mlp_outputs.append(mlp_output)
        mlp_detached_outputs.append(detached_outputs)
        ...
        combine_output, detached_outputs = callables.combine.forward(
            comm_stream, events[i], expert_output
        )
        expert_output = detached_outputs[0]
        combine_outputs.append(combine_output)
        combine_detached_outputs.append(detached_outputs)
        ...
        callables.attention.backward(
            ...
            *attention_outputs[prev_idx],
            attention_detached_outputs[prev_idx],
        )
        # f5. Post combine forward for current microbatch
        post_combine_output, detached_outputs = callables.post_combine.forward(
            comp_stream, events[i], expert_output, shared_expert_output, mlp_bias, hidden_states
        )
        post_combine_outputs.append(post_combine_output)
    ...
    callables.post_combine.backward(
        comp_stream,
        events[prev_idx],
        post_combine_outputs[microbatches - 1],
        torch.ones_like(post_combine_outputs[microbatches - 1]),
    )
    ...
    callables.combine.backward(
        ...
        *combine_outputs[microbatches - 1],
        combine_detached_outputs[microbatches - 1],
    )
    ...
    callables.mlp.backward(
        ...
        *mlp_outputs[microbatches - 1],
        mlp_detached_outputs[microbatches - 1],
    )
    ...
    callables.dispatch.backward(
        ...
        *dispatch_outputs[microbatches - 1],
        dispatch_detached_outputs[microbatches - 1],
    )
    ...
    callables.attention.backward(
        ...
        *attention_outputs[microbatches - 1],
        attention_detached_outputs[microbatches - 1],
    )
    torch.cuda.synchronize()
    capture = {"outputs": post_combine_outputs}
    for name, param in model.named_parameters():
        capture[name] = param.grad
...
def compare_captures(capture_a2a_overlap, capture_ref):
    ...
    for name, value in capture_ref.items():
        print(name, end='\0')
        assert (
            name in capture_a2a_overlap
        ), f"gradient name mismatch, '{name}' not in capture_a2a_overlap.keys()"
        if value is None:
            ...
            for i in range(len(value)):
                assert (
                    value[i].shape == capture_a2a_overlap[name][i].shape
                ), "outputs shape mismatch"
                # assert torch.allclose(value[i], capture_a2a_overlap[name][i]), f"outputs value mismatch at index {i}."
                assert bit_same(
                    value[i], capture_a2a_overlap[name][i]
                ), f"outputs value mismatch at index {i}."
            print(": PASS")
        else:
            try:
                assert (
                    value.shape == capture_a2a_overlap[name].shape
