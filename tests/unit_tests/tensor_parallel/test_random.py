...
def test_checkpoint_without_output_before_recompute_dependency():
    """A dependent checkpoint can restore its input before replaying."""

    def normal_forward(input_):
        x = torch.sin(input_)
        y = x * x
        return y.sum()

    def checkpoint_forward(input_):
        dependency = CheckpointWithoutOutput()
        dependent = CheckpointWithoutOutput()

        x = dependency.checkpoint(torch.sin, input_)
        y = dependent.checkpoint(lambda tensor: tensor * tensor, x)
        loss = y.sum()

        dependent.set_before_recompute(dependency.recompute_output)
        dependency.discard_output()
        dependent.discard_output_and_register_recompute(loss)
        return loss

    Utils.initialize_model_parallel()
    try:
        input1 = torch.randn((8, 8), device="cuda", requires_grad=True)
        input2 = input1.detach().clone().requires_grad_(True)
        ...
        output2.backward(retain_graph=True)
        assert torch.allclose(input1.grad, input2.grad)
    finally:
        Utils.destroy_model_parallel()
