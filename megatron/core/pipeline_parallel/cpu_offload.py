...
class ChunkOffloadHandler:
    ...
    def is_first_last_layer(self):
        ...
        """
        debug_rank(f"is_first_last_layer {self._is_first_last_vpp_chunk} {self.is_last_layer}")
    ...
    def bulk_offload_group(self, group_to_offload):
        ...
        debug_rank("exit bulk_offload_group")
    ...
    def on_group_commit_forward(self, release_tensors):
        ...
        debug_rank("exit on_group_commit_forward")
...
class GroupCommitFunction(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        debug_rank("GroupCommitFunction forward")
    ...
    def backward(ctx, *grad_output):
        # pylint: disable=missing-function-docstring
        debug_rank("GroupCommitFunction backward")
...
class GroupStartFunction(torch.autograd.Function):
    ...
    def forward(ctx, tensor, cpu_offload_handler, name):
        # pylint: disable=missing-function-docstring
        ctx.cpu_offload_handler = cpu_offload_handler
        debug_rank("GroupStartFunction forward")
    ...
    def backward(ctx, grad_output):
        # pylint: disable=missing-function-docstring
        debug_rank("GroupStartFunction backward")
        cpu_offload_handler = ctx.cpu_offload_handler
