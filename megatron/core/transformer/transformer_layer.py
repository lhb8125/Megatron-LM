...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def _submodule_attn_router_postprocess(
        self, node, residual, pre_mlp_layernorm_output, tokens_per_expert, local_tokens
    ):
    ...
    def get_submodule_callables(self, chunk_state):
        ...
        attn_func = get_func_with_default(
            self._submodule_attn_router_forward, self._submodule_attn_forward
        )
        ...
        attn_postprocess_func = get_func_with_default(
            self._submodule_attn_router_postprocess, self._submodule_non_moe_postprocess
        )
        ...
        dispatch_postprocess_func = get_func_with_default(
            self._submodule_dispatch_postprocess, self._submodule_non_moe_postprocess
        )
        ...
        mlp_postprocess_func = get_func_with_default(
            self._submodule_mlp_postprocess, self._submodule_non_moe_postprocess
        )
        ...
        combine_postprocess_func = get_func_with_default(
            self._submodule_combine_postprocess, self._submodule_non_moe_postprocess
        )
