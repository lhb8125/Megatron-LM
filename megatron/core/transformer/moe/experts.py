...
class TEGroupedMLP(MegatronModule):
    ...
    def __init__(
        ...
        self._use_grouped_tensor: bool = self.config.moe_use_grouped_tensor
        if self.config.fp8 or self.config.fp4 or self._use_grouped_tensor:
            ...
            align_size = (
                ...
                if (self._with_fused_impl or self._use_grouped_tensor)
                else None
            )
    ...
    def _apply_packed_bias(intermediate_parallel, packed_bias, tokens_per_expert, permuted_probs):
        ...
        # token count to create one bias row per permuted token. output_size avoids a stream
        # synchronization to compute sum(tokens_per_expert). Cast before repeating so both
        # forward arithmetic and repeat_interleave's backward reduction run in FP32.
    ...
    def _apply_bias(intermediate_parallel, bias_parallel, tokens_per_expert, permuted_probs):
        ...
        # and torch.split plus the Python zip below require concrete host token counts. Converting
        # a tensor with .tolist() synchronizes device data to the host, so this path must never be
        # included in a CUDA graph.
    ...
    def forward(
        ...
        # before expert compute (router padding, HybridEP/NCCL-EP fused dispatch/permute, or
        # DeepEP fused post-communication permutation). In those cases tokens_per_expert already
        # describes the padded segments, so leave both tensors unchanged.
        if skip_routed_expert_padding(self.config):
            pass
        # Regular AllToAll supplies unpadded expert segments and uses this fallback. FP8/FP4 need
        # recipe-specific alignment; the GroupedTensor backend uses aligned 256-token segments.
        ...
        if self._use_grouped_tensor:
            # GroupedTensor GEMMs consume CUDA int64 split metadata directly, so keep
            # tokens_per_expert on-device (no host round-trip / stream sync).
