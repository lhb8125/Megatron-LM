...
class MegatronFSDP(torch.nn.Module):
    ...
    Args:
        ...
        fsdp_ubr_enable_symmetric_rs (bool): Whether to use a same-offset registered staging
            buffer for eligible dense inner-FSDP reduce-scatter outputs. Defaults to False.
    ...
    def prepare_forward_module(self, module: nn.Module):
        ...
        window.  Clear the root marker only for that forward layer; recomputation
        in the independently scheduled backward layer remains ``PRE_BACKWARD``.
        """
