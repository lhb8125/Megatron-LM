...
- During an activation-checkpoint recompute, gathers only missing forward buffers owned
  directly by the fine-grained child module. `is_recomputing()`, rather than the shared
  FSDP `backward_phase`, selects this path so an overlapped normal forward remains full-unit.
