...
- **Full-CG rule**: post-backward waits on `rs_stream` only when that
  micro-batch submitted a reduction. A no-sync micro-batch must not capture a
  dependency on the side stream's eager history.
