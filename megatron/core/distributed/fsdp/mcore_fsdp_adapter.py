...
def _get_embedding_output_fsdp_units(module: nn.Module) -> List[nn.Module]:
    ...
    # endpoint. There is no combined root buffer to split in either case.
...
class FullyShardedDataParallel(_BaseDataParallel):
    ...
    def _init_with_fully_shard(
        ...
        # Untied embedding and output weights execute at opposite ends of a
        # full iteration. If both remain in the root FSDP unit, unsharding one
        # endpoint materializes both weights in a combined communication
        # buffer. Independent units let the CUDA graph private pool reuse
        # storage across their non-overlapping lifetimes. Their native wgrads
        # are not delayed, so normal post-backward hooks are safe.
        if (
